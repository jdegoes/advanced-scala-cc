 ...
 *
 * Type-level programming allows us to do computation using the type system,
 * often with the goal of increasing the type-safety of our applications, so we
 * can prevent broader classes of runtime errors.
 *
 * Scala 3 includes both old tools (refined types, givens, etc.) and new tools
 * (like match types, singleton types) to help us do type-level programming.
 */
...
object vectors:
   ...
   * EXERCISE 1
   *
   * Add a `++` operator to the `Vector` type, which appends two vectors
   * together, and tracks the size of the resulting vector at compile-time by
   * using compile-time operations.
   *
   * EXERCISE 2
   *
   * Add an append (`:+`) operator to the `Vector` type, which appends a single
...
object hlist:
   ...
   * EXERCISE 2
   *
   * Create a type class `At` that can retrieve a value at a specific index in
   * an HList. It should be a compile-time error if the index is out of bounds.
   */
  ...
  extension [H <: HList](hlist: H) def at[I <: Int & Singleton](idx: I)(using at: At[H, I]): at.Out = at(hlist, idx)
